...
### 步骤 4 — 比较页 ref 药丸 + 统计头微调

- [ ] ref 药丸三段式 `从 origin/main → 到 main`(替换原生 `<select>`)
- [ ] swap 交换按钮(当前缺失)
- [ ] 统计头说明句补"领先 N 个提交"(`main 领先 origin/main 2 个提交 · 4 文件改动`)
- [ ] 统计头字号 30px(现 34);(可选)位置移入左列滚动体内
- [ ] 左列宽 340(现可拖拽 288)—— 属功能增强,**保留**
