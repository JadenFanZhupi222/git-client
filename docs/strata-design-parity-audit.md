...
> **进度(2026-06-23 修复后)**:步骤 1–7 全部完成(分支 `feat/strata-parity`,7 个提交)。tsc 干净、64 tests 全绿、pnpm build 通过。下表"修复后"列为落地结果。

## 0. 结论速览

| 层 | 初始一致度 | 修复后 |
|---|---|---|
| 地基(token / 字体 / 玻璃材质 / 动效) | ~90% | ✅ 像素级 + diff 底色收敛回 token |
| 启动屏 EmptyState | ~95% | ✅(未动) |
| 顶栏 header | ~85% | ✅ 主页键可点回启动屏(主题键/高度按用户定保留) |
| 追溯 Blame | ~80% | ✅(未动,本就忠实) |
| 命令面板 | ~70% | ✅ 顶距/宽/遮罩 + 按组分区 + 行首图标(去 chip) |
| 比较 Compare | ~45% | ✅ ref 药丸 + 左列玻璃栏 + 统计头 |
| 历史 History | ~40% | ✅ 图谱节点/提交行/详情面板全面对齐 |
| 更改 Changes | ~35% | ✅ 玻璃栏 + 填充复选框 + 左脊 + 提交盒内盒 |
| 子模块/工作树/稀疏 | ~25% | ✅ 居中卡片表格 |
...
### 步骤 6 — 命令面板小修(`CommandPalette.tsx`)✅ 完成

- [x] 顶距 `mt-[14vh]`(原 12vh)
- [x] 宽度 `w-[35rem]` = 560px(原 34rem=544)
- [x] 遮罩色 `bg-[#05080d]/55`(原 `bg-black/40`)
- [x] "操作日志"归回"外观"组(`App.tsx` panel:oplog → group.appearance)
- [x] **按组分区 + 行首图标**(原型版式):命令模式下按 group 分桶,每组一个分区标题 + 命令行;
  `Command` 加 `icon` 字段(App.tsx 逐条赋图标),`CommandRow` 改为「左图标 + 标题」、去掉右侧分组 chip;
  键盘导航跳过分区标题行。搜索时按最佳匹配组序分区。(此前作为"保留 chip"跳过,经用户指出补做)

### 步骤 7 — 顶栏信息架构 ✅ 完成(已与用户确认)

> 用户确认:主题键**保留**在溢出菜单(顶栏减负的有意设计,不回退);顶栏高度**维持 44px**。

- [x] (用户定:不改)主题键保留在 MoreMenu —— 顶栏更干净,主题切换不高频
- [x] 主页键(BranchMark)可点击回启动屏:有仓库时点 logo = `setRepo(null)` 回启动屏 + hover 反馈;无仓库时纯装饰
- [x] (用户定:不改)顶栏高度维持 44px(`h-11`)
- i18n 新增 `header.home`(中英)

### 收尾 — token 纪律 ✅ 完成

- [x] diff add/del 行底色从 `bg-success/10`、`bg-danger/10` 收敛回 `--add-bg`/`--del-bg` 专用 token(`DiffView` UnifiedLine)
