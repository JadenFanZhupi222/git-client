...
- ✅ **M4.4 子模块感知**(已合 main):CliBackend `git submodule status`(解析行首 `-`/空格/`+`/`U`
  → 未初始化/已同步/未同步/冲突)+ `.gitmodules`(`config --file -z --list` 读 path↔url)合并出
  `SubmoduleInfo`;`update_submodule` 走 `git submodule update --init -- <path>`(init+checkout 一步,
  可联网→spawn_blocking)。model SubmoduleInfo/SubmoduleStatus + trait list/update_submodule(默认 Unsupported,
  Composite 委托 cli);DTO SubmoduleInfoDto(status 字符串 + short_sha)。竖切到新「子模块」标签——
  **仅当仓库确有子模块时出现**(useSubmodules 驱动 TabBar 显隐 + 切到无子模块仓库时退回「更改」);
...
### M5 · 更深的 diff 与历史 【✅ 已完成】
> 状态:六刀全部完成、merge 入 main 并 **push 到 origin**。每刀竖切 + 测试兜底 + 全门绿。
- ✅ M5.1 词级 diff(`similar` 行内高亮改动词)、✅ M5.2 并排(side-by-side)diff、
  ✅ M5.3 文件历史(`log --follow`)、✅ M5.4 行历史(`log -L`)、
  ✅ M5.5 pickaxe(`-S`/`-G`,接进历史搜索栏三模式)、✅ 图片 diff(并排预览新旧图)。
- spec 见 `docs/superpowers/specs/2026-06-{09,12}-*`。⚠️ 真机视觉验收仍待做。

### M6 · Polish & Harden(打磨与硬化)【当前 · 详见独立文档】
> M5 几刀为求快留了债(并排无同步滚动/折叠/虚拟化、图片 base64-in-JSON、新 CLI 读路径绕过缓存/取消、
> DTO 手工同步、新面板无键盘)。M6 不堆新功能,专门还债 + 打磨到世界级。
> **完整切片见 `docs/superpowers/plans/2026-06-12-m6-polish-harden.md`。**
- M6.1 并排 diff 体验(同步滚动 + 折叠未改区 + DiffView 虚拟化)、M6.2 图片去 base64(asset 协议)+ 对比模式、
  M6.3 新 CLI 读路径接缓存/取消、M6.4 specta 自动生成 TS 类型、M6.5 M5 测试补齐、M6.6 面板键盘/a11y。

### M7 · 协作 —— PR / 平台集成 【最后,面最大】(原 M6,顺延)
...
- 协作/PR(M7,原 M6 顺延)推后:面最大、且核心还没"无敌"前不值得分心。
