// 参考:合并进 app/src-tauri/src/lib.rs
// 脚手架生成的 lib.rs 里已经有一个 run() 函数,把下面的内容对照合并进去。
...
// 脚手架生成的 run() 里,把 .invoke_handler(...) 改成包含我们的命令:
//
// #[cfg_attr(mobile, tauri::mobile_entry_point)]
// pub fn run() {
//     tauri::Builder::default()
//         .plugin(tauri_plugin_dialog::init())   // 选目录用,需先加该插件
...
//
// 选目录的对话框需要 dialog 插件:
//   在 app/src-tauri 目录执行: cargo add tauri-plugin-dialog
//   并在前端: npm install @tauri-apps/plugin-dialog
